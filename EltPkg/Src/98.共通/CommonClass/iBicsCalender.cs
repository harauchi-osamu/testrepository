...
namespace CommonClass
{
    ...
    public class iBicsCalendar : System.Globalization.JapaneseCalendar
    {
        ...
        private static string GTABLE_NAME = CommonTable.DB.TBL_ERA.TABLE_NAME;
        private static string GTABLE_DBUSER = CommonTable.DB.DBConvert.TABLE_SCHEMA_DBCTR;

        private const string GM_NO = CommonTable.DB.TBL_ERA.SEQ;
        private const string GM_GENGO_KANJI = CommonTable.DB.TBL_ERA.KANJI;
        private const string GM_GENGO_SKANJI = CommonTable.DB.TBL_ERA.SHORTKANJI; 
        private const string GM_GENGO_ROMAN = CommonTable.DB.TBL_ERA.ROMAN;
        private const string GM_GENGO_SROMAN = CommonTable.DB.TBL_ERA.SHORTROMAN;
        private const string GM_CHANGE_VALUE = CommonTable.DB.TBL_ERA.TOWAREKIYEAR;
        private const string GM_SDATE = CommonTable.DB.TBL_ERA.STARTDATE;
        private const string GM_EDATE = CommonTable.DB.TBL_ERA.ENDDATE;
        ...
        public void SetHolidays()
        {
            ...
            {
                //HOLIDAYテーブルよりデータ取得
                _strQuery = CommonTable.DB.TBL_HOLIDAY.GetSelectQuery();

                DataRow[] drs = DB.DBManager.ExecuteRows(_strQuery, "HOLIDAY");
                mHolidays = new string[drs.Length];
                for (int i = 0; i < drs.Length; i++)
                {
                    mHolidays.SetValue(drs[i]["DAY"].ToString(), i);
                }
            }
            ...
            {
                //ERAテーブルよりデータ取得
                _strQuery = CommonTable.DB.TBL_ERA.GetSelectQuery();
                mDrGengo = DB.DBManager.ExecuteRows(_strQuery, GTABLE_NAME);
        ...
        // DELSTART 2022/07/08 銀行導入工程_不具合管理表No149 対応　不具合ありかつ未使用のため削除
        ///// <summary>
