...
namespace MainMenu
{
    static class AppMain
    {
        ...
        private static bool InitializeApp(string[] args, Controller ctl)
        {
            ...
            {
                // 業務設定取得
                // 引数の値を設定(0:共通、1:持出、2:持帰)
                AppInfo.Setting.SetGymId(ctl._GymID);
        ...
        private static void StartNavigate(Controller ctl)
        {
            try
            {
                ...
                // フォーム初期化
                SearchTeiseiHist.SearchTeiseiHist form = new SearchTeiseiHist.SearchTeiseiHist();
