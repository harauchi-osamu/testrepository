...
namespace SearchTxtView
{
    public class ImageHandler
    {
        ...
        /// イメージコントロールの作成
        /// </summary>
        public bool CreateImageControl(TBL_TRMEIIMG img, TBL_IMG_PARAM imgparam, string ImgFolderPath, bool isFit = false)
        {
            if (_canvas != null)
            {
                _canvas.Dispose();
            ...
            // 画像ファイル読み込み
            this.HasImage = false;
            string _orgFilePath = GetImgFilePath(ImgFolderPath, img);
        ...
        /// イメージファイルパスを取得する
        /// </summary>
        private string GetImgFilePath(string ImgFolderPath, TBL_TRMEIIMG img)
        {
            return Path.Combine(ImgFolderPath, img.m_IMG_FLNM);
        }
