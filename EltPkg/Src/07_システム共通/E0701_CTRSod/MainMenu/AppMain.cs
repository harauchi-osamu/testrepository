...
namespace MainMenu
{
    static class AppMain
    {
        ...
        private static bool StartControl(string[] args, Controller ctl)
        {
            ...
            // 設定ファイル読み込みでエラー発生時
            if (ctl.ExecMode == Controller.enumExecMode.SilentMode)
            {
                if (ctl.SettingData.ChkServerIni == false)
                {
                    LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format(CommonClass.ComMessageMgr.E00003), 3);
                    return false;
                }
                if (!string.IsNullOrEmpty(ctl.SettingData.CheckParamMsg))
                {
                    LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format(CommonClass.ComMessageMgr.E01001, ctl.SettingData.CheckParamMsg), 3);
                    return false;
                }
            }
            ...
            {
                if (ctl.ExecMode != Controller.enumExecMode.SilentMode) ComMessageMgr.MessageError(ComMessageMgr.E00005, ex.Message);
        ...
        private static bool CheckArgs(string[] args, Controller ctl)
        {
            try
            {
                ...
                if (args.Length < 2)
                {
                    if (ctl.ExecMode != Controller.enumExecMode.SilentMode) ComMessageMgr.MessageError(ComMessageMgr.E00002);
            ...
            {
                if (ctl.ExecMode != Controller.enumExecMode.SilentMode) MessageBox.Show("引数取得処理でエラーが発生しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private static bool InitializeApp(string[] args, Controller ctl)
        {
            try
            {
                ...
                {
                    // サイレントモードの場合
                    AppInfo.Setting.SetSchemaBankCD(int.Parse(NCR.Server.ContractBankCd));
                }
                else
                {
                    //画面表示
                ...
                {
                    if (ctl.ExecMode != Controller.enumExecMode.SilentMode) CommonClass.ComMessageMgr.MessageError(CommonClass.ComMessageMgr.E01002);
            ...
            {
                if (ctl.ExecMode != Controller.enumExecMode.SilentMode) MessageBox.Show("アプリケーション初期化処理でエラーが発生しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private static bool StartNavigate(Controller ctl)
        {
            try
            {
                ...
                {
                    // サイレントモードの場合
                    StartDayUpdate update = new StartDayUpdate(ctl);
                    if (!update.StartProc())
                    {
                        return false;
                    }
                ...
                else
                {
                    ...
                    // フォーム初期化
                    StartDayOperation.StartDayOperation form = new StartDayOperation.StartDayOperation();
            ...
            {
                if (ctl.ExecMode != Controller.enumExecMode.SilentMode) MessageBox.Show("画面表示処理でエラーが発生しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                LogWriter.writeLog(MethodBase.GetCurrentMethod(), "処理実行失敗: " + ex.ToString(), 3);
                return false;
            }
        }
