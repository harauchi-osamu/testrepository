    ...
    static class AppMain
    {
        ...
        private static void StartNavigate(Controller ctl)
        {
            try
            {
                ...
                //if (form.bankStatus == EntryForm.BankStatus.他行)
                //{
                //    // 他行・編集不可の場合メッセージを表示して終了
                //    ComMessageMgr.MessageWarning("持帰銀行コードが当行でないため、不渡エントリはできません。");
                //    form = null;
                //    return;
                //}
                //else if (form.regErrStatus == EntryForm.RegistErrStatus.不渡編集不可)
                //{
                //    // その他・編集不可の場合メッセージを表示して終了
                //    ComMessageMgr.MessageWarning("不渡返還登録編集不可のため、不渡エントリはできません。");
                //    form = null;
                //    return;
                //}

                //if (form.regErrStatus == EntryForm.RegistErrStatus.持出取消編集不可)
                //{
                //    // 編集不可の場合メッセージを表示して終了
                //    ComMessageMgr.MessageWarning("持出取消済のため、不渡エントリはできません。");
                //    form = null;
                //    return;
                //}
                //else if (form.regErrStatus == EntryForm.RegistErrStatus.持帰訂正編集不可 || form.bankStatus == EntryForm.BankStatus.他行)
                //{
                //    // 他行・編集不可の場合メッセージを表示して終了
                //    ComMessageMgr.MessageWarning("持帰銀行コードが当行でないため、不渡エントリはできません。");
                //    form = null;
