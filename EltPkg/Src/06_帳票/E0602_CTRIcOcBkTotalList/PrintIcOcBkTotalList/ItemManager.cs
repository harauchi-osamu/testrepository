...
namespace PrintIcOcBkTotalList
{
    ...
    public class ItemManager : ManagerBase
    {
        ...
        public bool GetPrintData(out List<PrintDetail> detail, FormBase form = null)
        {
            ...
            // SELECT実行
            string strSQL = SQLPrint.GetIcOcBkTotalPrintData(AppInfo.Setting.GymId, DispParams.Date, AppInfo.Setting.SchemaBankCD);
            using (AdoDatabaseProvider dbp = GenDbProviderFactory.CreateAdoProvider1())
            {
                try
                {
        ...
        {
            return System.IO.Path.Combine(string.Format(NCR.Server.ReportPath, AppInfo.Setting.SchemaBankCD), "CTRIcOcBkTotalList.rpt");
        }
