...
namespace PrintSyubetuList
{
    ...
    /// 持出バッチ別合計表画面
    /// </summary>
    public partial class PrintSyubetuList : EntryCommonFormBase
    {
        private Controller _ctl = null;
        private MasterManager _masterMgr = null;
        private ItemManager _itemMgr = null;
        private CTRSyubetuListDataSet _ds = null;
        ...
        /// コンストラクタ
        /// </summary>
        public PrintSyubetuList()
        {
            InitializeComponent();
            _ds = new CTRSyubetuListDataSet();
        }
        ...
        private void PrintList(bool isPreview)
        {
            ...
            // ヘッダー
            CTRSyubetuListDataSet.HeaderDataTable hTable = _ds.Header;
            CTRSyubetuListDataSet.HeaderRow hRow = hTable.NewHeaderRow();
            hRow.業務 = "業務：交換持帰";
            //作成日
            hRow.作成日 = CommonUtil.ConvToDateFormat(AplInfo.OpDate(), 3);
            //交換日
            hRow.交換日 = CommonUtil.ConvToDateFormat(_itemMgr.DispParams.Date, 3);
            hTable.AddHeaderRow(hRow);

            // フッター
            CTRSyubetuListDataSet.FooterDataTable fTable = _ds.Footer;
            CTRSyubetuListDataSet.FooterRow fRow = fTable.NewFooterRow();
        ...
        {
            // 明細
            CTRSyubetuListDataSet.DetailDataTable dTable = _ds.Detail;
            foreach (var Grpdetail in details.GroupBy(x => new { x.SyuruiCode, x.SyuruiName }).OrderBy(x => x.Key.SyuruiCode))
            {
                //明細データ処理
                CTRSyubetuListDataSet.DetailRow dRow = dTable.NewDetailRow();
                // 手形種類コード
                dRow.手形種類コード = Grpdetail.Key.SyuruiCode.ToString("D3");
                // 手形種類名
                dRow.手形種類名 = Grpdetail.Key.SyuruiName;
                // 明細総枚数
                dRow.明細総枚数 = Grpdetail.Sum(x => x.Count);
                // 明細総金額
                dRow.明細総金額 = Grpdetail.Sum(x => x.Amount);
                // 追加
        ...
        private bool CheckDate()
        {
            ...
            {
                this.SetStatusMessage(string.Format(CommonClass.ComMessageMgr.E02001, lblInputDATE.Text));
                return false;
            }
