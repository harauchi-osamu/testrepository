...
namespace PrintIcBrTotalList
{
    ...
    public class ItemManager : ManagerBase
    {
        ...
        public bool GetPrintData(out List<PrintDetail> detail, FormBase form = null)
        {
            ...
            // SELECT実行
            string strSQL = SQLPrint.GetCTRIcBrTotalPrintData(AppInfo.Setting.GymId, DispParams.Date, AppInfo.Setting.SchemaBankCD);
            using (AdoDatabaseProvider dbp = GenDbProviderFactory.CreateAdoProvider1())
            {
                try
                {
        ...
        {
            return System.IO.Path.Combine(string.Format(NCR.Server.ReportPath, AppInfo.Setting.SchemaBankCD), "CTRIcBrTotalList.rpt");
        }
        ...
        public class PrintDetail
        {
            ...
            {
                BRNo = DBConvert.ToIntNull(dr["BR_NO"]);
                BRName = DBConvert.ToStringNull(dr["BR_NAME_KANJI"]);
