...
namespace ImageImportTotal
{
    ...
    public partial class SearchListForm : EntryCommonFormBase
    {
        ...
        {
            base.SetDispName2("イメージ取込　合計票一覧");
        }
        ...
        {
            if (!_itemMgr.FetchTotalControl(this))
            {
                return;
            }
            ...
            ListViewItem[] listView = new ListViewItem[_itemMgr.scan_total.Count];
            foreach (KeyValuePair<string, TBL_BR_TOTAL> Data in _itemMgr.scan_total)
            {
                string key = Data.Key;
                string OPERATION_DATE = Data.Value._OPERATION_DATE.ToString();
                string IMG_NAME = Data.Value._SCAN_IMG_FLNM;
                string OC_BK_NO = DispDataFormatZero(Data.Value, Data.Value.m_BK_NO, "D4");
                string OC_BR_NO = DispDataFormatZero(Data.Value, Data.Value.m_BR_NO, "D4");
                string SCAN_DATE = CommonUtil.ConvToDateFormat(DispDataFormatZero(Data.Value, Data.Value.m_SCAN_DATE, "D8"), 3);
                string SCAN_BR_NO = DispDataFormatZero(Data.Value, Data.Value.m_SCAN_BR_NO, "D4");
                string TOTAL_COUNT = DispDataFormatZero(Data.Value, Data.Value.m_TOTAL_COUNT, "#,##0");
                string TOTAL_AMOUNT = DispDataFormatZero(Data.Value, Data.Value.m_TOTAL_AMOUNT, "#,##0");
                string STATUS = Data.Value.m_STATUS.ToString();
                string STATUSTEXT = TBL_BR_TOTAL.GetStatusText((TBL_BR_TOTAL.enumStatus)Data.Value.m_STATUS);
                string LOCK = Data.Value.m_LOCK_TERM;

                listItem.Clear();
                listItem.Add(key);
                listItem.Add(OPERATION_DATE);
                listItem.Add(IMG_NAME);
                ...
                //色設定
                if (ListColor.ContainsKey(Data.Value.m_STATUS) )
                {
                    listView[cnt].UseItemStyleForSubItems = true;
                    listView[cnt].BackColor = System.Drawing.Color.FromArgb(ListColor[Data.Value.m_STATUS]);
                }

                // 前選択状態の復元
                if (IMG_NAME == _itemMgr.ListDispParams.FileName && Data.Value._OPERATION_DATE == _itemMgr.ListDispParams.OperationDate)
                {
                    SelectItem = cnt;
                }
        ...
        private Dictionary<int,int> GetListColor()
        {
            ...
            foreach (string Data in this._ctl.SettingData.TotalListBackColor.Split(','))
            {
        ...
        protected override void btnFunc06_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (_itemMgr.ListDispParams.FileSelected)
                {
                    // 選択行がある場合、選択行以降のデータを取得するため
                    // 状態順に表示されるため対象が保留以上の場合は自動取得データがないので処理終了
                    if (DBConvert.ToIntNull(_itemMgr.ListDispParams.Status) >= (int)TBL_BR_TOTAL.enumStatus.Hold)
                    {
                        return;
                    }
                }
                ...
                // Autoモード設定
                _itemMgr.InputParams.Mode = ItemManager.TotalInputParams.InputMode.Auto;

                // INPUT画面表示
                TotalInputForm form = new TotalInputForm();
        ...
        protected override void btnFunc07_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!(_itemMgr.ListDispParams.Status == TBL_BR_TOTAL.enumStatus.Wait.ToString("d") ||
                    _itemMgr.ListDispParams.Status == TBL_BR_TOTAL.enumStatus.Hold.ToString("d") ||
                    _itemMgr.ListDispParams.Status == TBL_BR_TOTAL.enumStatus.Delete.ToString("d")))
                {
                    // 処理待ち・保留・削除以外の場合
                    return;
                }
                ...
                // Selectモード設定
                _itemMgr.InputParams.Mode = ItemManager.TotalInputParams.InputMode.Select;
                ...
                {
                    // データがない場合
                    this.SetStatusMessage("合計票入力できないイメージです。最新状態を確認してください。");
                }
                else
                {
                    // 上記以外の場合画面表示
        ...
        protected override void btnFunc09_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //ステータスチェック
                if (_itemMgr.ListDispParams.Status != TBL_BR_TOTAL.enumStatus.Processing.ToString("D"))
                {
                    //処理中以外
                    ComMessageMgr.MessageWarning("選択ファイルはロック解除対象外です");
                    return;
                }
        ...
        private bool GetSelectListData()
        {
            ...
            string[] Keys = CommonUtil.DivideKeys(this.lvBatList.SelectedItems[0].SubItems[KEY].Text, "|");
            _itemMgr.ListDispParams.OperationDate = int.Parse(Keys[0]);
            _itemMgr.ListDispParams.FileName = Keys[1];
            _itemMgr.ListDispParams.Status = this.lvBatList.SelectedItems[0].SubItems[STATUS].Text;

            // 自動選択時の初期設定情報
            _itemMgr.ListDispParams.FileSelected = true;
            _itemMgr.ListDispParams.AutoSelectOpeDate = _itemMgr.ListDispParams.OperationDate;
            _itemMgr.ListDispParams.AutoSelectFileName = _itemMgr.ListDispParams.FileName;
