...
namespace ImageImportFutai
{
    ...
    public class TRManager
    {
        ...
        private List<TBL_SCAN_MEI> scan_mei
        {
            get { return _itemMgr.scan_mei.Values.ToList(); }
        }

        /// <summary>スキャン明細連番一覧データ</summary>
        private List<int> scanmei_renban
        {
            get { return _itemMgr.scanmei_renban; }
        }

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public TRManager(Controller ctl, int BatchNumber, string ImgFolderPath)
        {
            _ctl = ctl;
            ...
            // DB登録クラス作成
            _TRAccess = new ImportTRAccess(_ctl.SettingData.OCRSettingData, AssignParams.GymDate, _BatchNumber,
        ...
        public bool TRDataInput()
        {
            ...
            // バッチフォルダ名の算出
            string BachtFolderPath = Path.Combine(string.Format(NCR.Server.BankFutaiImageRoot, InputBatchData.m_OC_BK_NO), 
                                                  AppInfo.Setting.GymId.ToString("D3") + AssignParams.GymDate.ToString("D8") + _BatchNumber.ToString("D8"));
            ...
            using (AdoNonCommitTransaction Tran = new AdoNonCommitTransaction(dbp))
            {
                try
                {
                    ...
                    //表・裏のデータ設定枠を作成
                    BatchCtl.FileList.Add(new ImportFileAccess.FileCtl(ImportFileAccess.FileCtl.ImgKbn.Omote, InputParams.BatchImage.Front));
                    BatchCtl.FileList.Add(new ImportFileAccess.FileCtl(ImportFileAccess.FileCtl.ImgKbn.Ura, InputParams.BatchImage.Back));
                    ...
                    // 証券単位での処理
                    foreach (int renban in scanmei_renban)
                    {
                        string FrontName = string.Empty;
                        ImportFileAccess.DetailCtl Detail = new ImportFileAccess.DetailCtl();
                        RenameList.Add(renban.ToString(), Detail);

                        foreach (TBL_SCAN_MEI Data in scan_mei.Where(m => m.m_BATCH_UCHI_RENBAN == renban))
                        {
                            if (Data.m_IMG_KBN == (int)ImportFileAccess.FileCtl.ImgKbn.Omote)
                            {
                                FrontName = Data._IMG_NAME;
                            }

                            //データ設定枠を作成
                            Detail.FileList.Add(new ImportFileAccess.FileCtl((ImportFileAccess.FileCtl.ImgKbn)Data.m_IMG_KBN, Data._IMG_NAME));
                        }

                        //対象ファイルのOCRデータ取得
                        List<TBL_OCR_DATA> OCRList = _itemMgr.GetMeiOCRData(FrontName, _itemMgr.InputBatchData.m_SCAN_DATE, _itemMgr.InputParams.TargetBatchFolderName, dbp);
                        ...
                        //明細トランザクションの登録
                        if (!_TRAccess.InsTRMei(TermIPAddress, renban, FrontName, out int DspID, OCRList, dbp, Tran))
                        {
                            //catch でリネームの戻し作業等を行う
                        ...
                        //明細イメージトランザクションの登録
                        if (!_TRAccess.InsTRMeiImage(TermIPAddress, renban, Detail, dbp, Tran))
                        {
                            //catch でリネームの戻し作業等を行う
                        ...
                        //項目トランザクションの登録
                        if (!_TRAccess.InsTRItemData(TermIPAddress, renban, FrontName, DspID, OCRList, out bool BankComp, out bool AmtComp, dbp, Tran))
                        {
                            //catch でリネームの戻し作業等を行う
                        ...
                        //補正ステータスの登録
                        if (!_TRAccess.InsTRHoseiData(TermIPAddress, renban, BankComp, AmtComp, dbp, Tran))
                        {
                            //catch でリネームの戻し作業等を行う
