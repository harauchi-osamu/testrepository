...
namespace ImageImportNormal
{
    ...
    public partial class BatchInputForm : EntryCommonFormBase
    {
        ...
        protected override void SetDisplayParams()
        {
            ...
            // バッチ票イメージ表示
            if (!DispBatchImage(_itemMgr.InputParams.BatchImage.Front, pbScanImage, this))
            {
        ...
        protected override void btnFunc12_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //確認メッセージ表示
                if (ComMessageMgr.MessageQuestion(MessageBoxButtons.YesNo, MessageBoxDefaultButton.Button2, "バッチを確定してもよろしいですか？") == DialogResult.No)
                {
                    return;
                }
        ...
        private bool BatchInput()
        {
            ...
            // イメージ一覧を作成
            IEnumerable<string> FileList = Directory.EnumerateFiles(ImgFolderPath, "*").Select(name => Path.GetFileName(name)).OrderBy(name => name);
            Dictionary<string, ItemManager.BatchInputParams.ImageData> ImageList;
            if (_itemMgr.InputParams.GrpPattern)
            {
                // グループ設定あり
                ImageList = MakeImageListGrp(FileList);
            }
            else if (_itemMgr.InputParams.Pattern.SerNoFlg == 1)
            {
                // グループ設定なしで連番フラグあり
            ...
            {
                // グループ設定なしで連番フラグなし
                ImageList = new Dictionary<string, ItemManager.BatchInputParams.ImageData>();
            }

            // イメージ一覧の表裏の設定済チェック
            foreach (ItemManager.BatchInputParams.ImageData Data in ImageList.Values)
            {
                if (!Data.ChkFBInput())
                {
                    //表裏両方に入力がない箇所がある場合はエラーを表示して画面に戻る。
                    //エラーを表示して画面に戻る。
                    CommonClass.ComMessageMgr.MessageWarning("取得したイメージファイルの組み合わせに不整合あります");
                    return false;
                }
            }

            // 組み合わせ数とスキャン枚数の整合性チェック
            if (ImageList.Count != _itemMgr.InputBatchData.m_SCAN_COUNT)
            {
                //エラーを表示して画面に戻る。
                CommonClass.ComMessageMgr.MessageWarning("スキャン枚数入力とイメージ数が一致しません");
                return false;
            ...
            {
                if (_itemMgr.GetBatchNumber(AplInfo.OpDate(), _itemMgr.InputBatchData.m_OC_BK_NO, out BatchNumber))
                {
                    break;
                }
            }
            ...
            // 登録処理
            TRManager insTR = new TRManager(_ctl, AplInfo.OpDate(), BatchNumber, ImgFolderPath, FolderFileCount, ImageList);
            ...
            if (!ImportFileAccess.ImageBackUp(ImgFolderPath, NCR.Server.ScanImageBackUpRoot))
            {
                ...
                CommonClass.ComMessageMgr.MessageError("イメージ退避処理でエラーが発生しました");
            }
        ...
        private void NextInput(TBL_SCAN_BATCH_CTL.enumStatus status)
        {
            ...
            else
            {
                ...
                {
                    //エラーを表示して一覧画面に戻る。
                    CommonClass.ComMessageMgr.MessageError("データの取得に失敗しました");
                    this.DialogResult = DialogResult.Abort;
                    this.Close();
                    return;
                }
        ...
        private void GetBatchData()
        {
            ...
            if (!GetFilePattern(FileList))
            {
                _itemMgr.InputParams.DispMode = ItemManager.BatchInputParams.DispErrMode.PatternNoData;
                return;
            }
        ...
        /// ファイル名パターン確認
        /// </summary>
        private bool GetFilePattern(IEnumerable<string> FileList)
        {
            foreach (string name in FileList)
            {
                foreach (SettingData.ScanImage Pattern in _ctl.SettingData.ScanImagePattern)
                {
                    Match mct = Regex.Match(name, Pattern.Pattern);

                    if (mct.Success)
                    {
                        // グループ化が複数は想定外のため不一致扱い
                        if (mct.Groups.Count > 2)
                        {
                            continue;
                        }

                        // 対象情報設定
                        _itemMgr.InputParams.Pattern = Pattern;
                        if (mct.Groups.Count > 1)
                        {
                            _itemMgr.InputParams.GrpPattern = true;
                        }
        ...
        /// (グループ化あり)
        /// </summary>
        private Dictionary<string, ItemManager.BatchInputParams.ImageData> MakeImageListGrp(IEnumerable<string> FileList)
        {
            ...
            foreach (string name in FileList)
            {
                Match mct = Regex.Match(name, _itemMgr.InputParams.Pattern.Pattern);
                if (!mct.Success)
                {
                    continue;
                }
                ...
                for (int i = mct.Groups.Count - 1; i > 0; i--)
                {
                    if (mct.Groups[i].Value == _itemMgr.InputParams.Pattern.KeyFront)
                    {
                        IType = ImageType.Front;
                        break;
                    }

                    if (mct.Groups[i].Value == _itemMgr.InputParams.Pattern.KeyBack)
                    {
                        IType = ImageType.Back;
                        break;
                    }
                }

                if (IType == ImageType.None)
                {
                    //表裏キーワードに一致しない場合は次の処理
                    continue;
                }

                // グループ化している箇所を除いたキーファイル名を作成
                string grpstr = name;
                for (int i = mct.Groups.Count - 1; i > 0; i--)
                {
                    grpstr = grpstr.Remove(mct.Groups[i].Index, mct.Groups[i].Length);
                }

                // 表裏情報の登録
                ItemManager.BatchInputParams.ImageData ImgData;
                if (ImageList.ContainsKey(grpstr))
                {
                    // キーファイル名で登録済の場合
                    ImgData = ImageList[grpstr];
                }
                else
                {
                    // キーファイル名で未登録の場合
                    ImgData = new ItemManager.BatchInputParams.ImageData();
                    ImageList.Add(grpstr, ImgData);
                }

                if (IType == ImageType.Front)
                {
                    ImgData.Front = name;
                }
                else if (IType == ImageType.Back)
                {
                    ImgData.Back = name;
                }
            }
        ...
        private string GetDispModeErrMsg()
        {
            ...
            case ItemManager.BatchInputParams.DispErrMode.FolderNotExists:
                ...
                break;
            case ItemManager.BatchInputParams.DispErrMode.PatternNoData:
                // バッチフォルダが存在しない場合
                ErrMsg = "パターン定義と一致するファイルが存在しませんでした";
                break;
        ...
        /// バッチ票イメージ表示
        /// </summary>
        private bool DispBatchImage(string ImagePath, PictureBox DispPicture, FormBase form = null)
        {
            //バッチイメージ表示
            if (!_itemMgr.InputParams.BatchImage.ChkFrontInput())
            {
        ...
        private bool CheckTOTAL_COUNT()
        {
            ...
            // 整合性チェック
            if ((ChkIntText + 1) != txtSCAN_COUNT.getInt())
            {
                this.SetStatusMessage(string.Format("スキャン枚数と合計枚数が不整合です"));
                return false;
            }
