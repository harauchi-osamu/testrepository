...
namespace ImageImportNormal
{
    ...
    public class ItemManager : ManagerBase
    {
        ...
        /// 処理待更新SQL実行(一覧画面)
        /// </summary>
        public int UpdateStatusWaitSearchList(FormBase form = null)
        {
        ...
        public int UpdateStatusComplete(AdoDatabaseProvider dbp, AdoNonCommitTransaction Tran, FormBase form = null)
        {
            ...
            try
            {
                string strSQL = InputBatchData.GetUpdateQueryComplete(TBL_SCAN_BATCH_CTL.enumStatus.Processing);
        ...
        public string GetBatchOCRData(string field_name, FormBase form = null)
        {
            using (AdoDatabaseProvider dbp = GenDbProviderFactory.CreateAdoProvider1())
            {
                try
                {
                    ...
                    // 対象データの取得
                    string strSQL = TBL_OCR_DATA.GetSelectQuery(AppInfo.Setting.GymId, (int)TBL_OCR_DATA.OCRInputRoute.Normal, 
        ...
        public List<TBL_OCR_DATA> GetMeiOCRData(string file_name, int scandate, string batchfolderName, AdoDatabaseProvider dbp)
        {
            ...
            {
                // 対象データの取得
                string strSQL = TBL_OCR_DATA.GetSelectQueryDirImgName(AppInfo.Setting.GymId, (int)TBL_OCR_DATA.OCRInputRoute.Normal, scandate, batchfolderName, file_name);
                DataTable tbl = dbp.SelectTable(strSQL, new List<IDbDataParameter>());
                for (int i = 0; i < tbl.Rows.Count; i++)
                {
        ...
        {
            return System.IO.Path.Combine(NCR.Server.ScanNormalImageRoot, InputParams.TargetBatchFolderName);
        }
        ...
        public class ListDisplayParams
        {
            ...
            public TBL_SCAN_BATCH_CTL.InputRoute Route { get; set; } = TBL_SCAN_BATCH_CTL.InputRoute.Normal;
        ...
        public class BatchInputParams
        {
            ...
            public TBL_SCAN_BATCH_CTL.InputRoute TargetRoute { get; set; } = TBL_SCAN_BATCH_CTL.InputRoute.Normal;
            public string TargetBatchFolderName { get; set; } = string.Empty;

            public SettingData.ScanImage Pattern { get; set; } = new SettingData.ScanImage();
            public bool GrpPattern { get; set; } = false;
            ...
            {
                Mode = Mode;
                TargetRoute = TBL_SCAN_BATCH_CTL.InputRoute.Normal;
                TargetBatchFolderName = string.Empty;
                Pattern = new SettingData.ScanImage();
                GrpPattern = false;
