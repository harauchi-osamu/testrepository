...
namespace CTROcCancelMk
{
    ...
    public partial class FileCreateForm : EntryCommonFormBase
	{
        ...
        protected override void SetDisplayParams()
        {
            ...
            // データ取得
            if (!_itemMgr.FetchTrMeiOC(ItemManager.CreateType.全データ, this))
            {
                return;
            }

            // 予定
            _itemMgr.DispParams.YoteiCancelCnt = _itemMgr.OCMeisaiList2.Count;
            ...
            LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format("予定件数 取消={0}件",
                _itemMgr.DispParams.YoteiCancelCnt), 1);
        ...
        {
            // 結果
            int resEltCnt = _itemMgr.OCMeisaiList2.Count;
            int resOwnCnt = _itemMgr.ICMeisaiOwnList2.Count;
            ...
            LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format("結果件数 電子交換 取消={0}件", resEltCnt), 1);
            LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format("結果件数 行内交換 取消={0}件", resOwnCnt), 1);
        ...
        protected override void btnFunc12_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // 完了メッセージ
                    int total = 0;
                    total += _itemMgr.OCMeisaiList2.Count;
                    total += _itemMgr.ICMeisaiOwnList2.Count;
                    if (total > 0)
                    {
                        string msg = ComMessageMgr.Msg(ComMessageMgr.I00001, "持出取消テキスト作成");
