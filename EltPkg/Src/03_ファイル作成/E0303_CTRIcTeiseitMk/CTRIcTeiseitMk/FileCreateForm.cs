...
namespace CTRIcTeiseitMk
{
    ...
    public partial class FileCreateForm : EntryCommonFormBase
	{
        ...
        {
            base.SetDispName2("持帰証券データ訂正テキスト作成");
        }
        ...
        protected override void SetDisplayParams()
        {
            ...
            // データ取得
            if (!_itemMgr.FetchTrMeiIC(ItemManager.DataType.訂正データ, ItemManager.CreateType.全データ, this))
            {
                return;
            }
            ...
            // 予定
            _itemMgr.DispParams.YoteiTeiseiCnt = _itemMgr.ICMeisaiList1.Count;
            _itemMgr.DispParams.YoteiCancelCnt = _itemMgr.ICMeisaiList2.Count;
            _itemMgr.DispParams.YoteiTotalCnt = _itemMgr.DispParams.YoteiTeiseiCnt + _itemMgr.DispParams.YoteiCancelCnt;
            lblYoteiTeiseiCnt.Text = string.Format("{0:###,##0}", _itemMgr.DispParams.YoteiTeiseiCnt);
            ...
            LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format("予定件数 訂正={0}件、取消={1}件",
                _itemMgr.DispParams.YoteiTeiseiCnt, _itemMgr.DispParams.YoteiCancelCnt), 1);
        ...
        private void SetDisplayParamsResult()
        {
            ...
            LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format("結果件数 電子交換 訂正={0}件、取消={1}件、合計={2}件",
                _itemMgr.ICMeisaiList1.Count, _itemMgr.ICMeisaiList2.Count, resEltCnt), 1);
            LogWriter.writeLog(MethodBase.GetCurrentMethod(), string.Format("結果件数 行内交換 訂正={0}件、取消={1}件、合計={2}件",
        ...
        protected override void btnFunc12_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (total > 0)
                    {
                        string msg = ComMessageMgr.Msg(ComMessageMgr.I00001, "持帰証券データ訂正テキスト作成");
