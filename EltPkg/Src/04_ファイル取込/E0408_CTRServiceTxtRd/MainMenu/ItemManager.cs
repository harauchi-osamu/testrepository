...
namespace MainMenu
{
    ...
    public class ItemManager : ManagerBase
    {
        ...
        /// 対象の作成日を取得
        /// </summary>
        private void GetMakeDate(out int MkDate)
        {
            MkDate = int.Parse(_TargetFilename.Substring(12, 8));
        }
        ...
        /// 別プロセスの実行
        /// </summary>
        public int RunProcess(string ExeName, string Argument, FormBase form = null)
        {
            string WorkDir = string.Format(NCR.Server.ExePath, AppInfo.Setting.SchemaBankCD);
            return ProcessManager.RunProcess(Path.Combine(WorkDir, ExeName), WorkDir, Argument, true, false);
        }
