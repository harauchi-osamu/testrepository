...
namespace MainMenu
{
    ...
    public class Controller : ControllerBase
    {
        ...
        protected override bool CheckServerIni()
        {
            ...
            ChkParam(NCR.Server.BankCheckImageRoot, "持帰ダウンロード確定前イメージルート(銀行別)");
            ChkParam(NCR.Server.ScanImageICOCRRoot, "持帰OCR取込用ルートフォルダ");
            ChkParam(NCR.Server.ICOCRCopyUnitCount, "持帰OCRコピー件数単位");
            // OCRオプション導入はZeroOK
