...
namespace MainMenu
{
	static class AppMain
	{
		...
		static void Main(string[] args)
		{
            ...
            try
			{
                type = StartControl(args, new Controller());
            }
            finally
			{
				Close();
		...
		private static Type StartControl(string[] args, Controller ctl)
		{
			// 引数チェック
            ...
            // ファイル取込機能については所定回数リトライする
            bool Connect = false;
            for (int i = 1; i <= ctl.RetryCount; i++)
            {
                // DB初期化
                try
                {
                ...
                //指定時間スリープ
                System.Threading.Thread.Sleep(ctl.SleepTime);
            }
            if (!Connect)
            {
                LogWriter.writeLog(MethodBase.GetCurrentMethod(), "DB接続に失敗しました", 3);
                return Type.Warning;
            }
            ...
            for (int i = 1; i <= ctl.RetryCount; i++)
            {
                if (System.IO.Directory.Exists(NCR.Server.ReceiveRoot))
                {
                    FolderChk = true;
                    break;
                }
                LogWriter.writeLog(MethodBase.GetCurrentMethod(), "ファイルサーバー接続不可", 3);
            ...
            if (!FolderChk)
            {
                LogWriter.writeLog(MethodBase.GetCurrentMethod(), "ファイルサーバーが確認できません", 3);
                return Type.Warning;
            }
		...
		/// </summary>
		/// <param name="ctl"></param>
		private static Type StartNavigate(Controller ctl)
		{
			try
			{
				// マスタテーブル取得
                ...
                //処理実行
                FileImport fileImport = new FileImport(ctl);
